...
export async function execute(interaction) {
    ...
    if (subcommand === 'welcome') {
        ...
        await guildService.updateSetting(guildId, 'welcome_channel_id', channel.id);
        await interaction.reply(`✅ **Welcome Channel** berhasil diset ke ${channel}`);
    }
    else if (subcommand === 'logs') {
        const channel = interaction.options.getChannel('channel');
        await guildService.updateSetting(guildId, 'log_channel_id', channel.id);
        await interaction.reply(`✅ **Log Channel** berhasil diset ke ${channel}`);
    }
    else if (subcommand === 'autorole') {
        const role = interaction.options.getRole('role');
        await guildService.updateSetting(guildId, 'auto_role_id', role.id);
        await interaction.reply(`✅ **Auto Role** berhasil diset ke **${role.name}**`);
    }
    else if (subcommand === 'levelup') {
        const channel = interaction.options.getChannel('channel');
        await guildService.updateSetting(guildId, 'levelup_channel_id', channel.id);
        ...
        const channel = interaction.options.getChannel('channel');
        await guildService.updateSetting(guildId, 'game_source_channel_id', channel.id);
        ...
        const channel = interaction.options.getChannel('channel');
        await guildService.updateSetting(guildId, 'request_channel_id', channel.id);
        ...
        const channel = interaction.options.getChannel('channel');
        await guildService.updateSetting(guildId, 'news_channel_id', channel.id);
        ...
        const channel = interaction.options.getChannel('channel');
        await guildService.updateSetting(guildId, 'general_chat_channel_id', channel.id);
        ...
        const channel = interaction.options.getChannel('channel');
        await guildService.updateSetting(guildId, 'alarm_channel_id', channel.id);
        ...
        await guildService.updateSetting(guildId, 'alarm_schedule', jam);
